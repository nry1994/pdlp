...
function evaluate_unscaled_iteration_stats(
  ...
  return compute_stats(
    ...
    original_dual_delta, current_omega,
  )
end
...
function compute_stats(
  ...
  primal_feasibility = norm(
    -lp.b +
    lp.A * primal_solution, 2) / (norm(lp.b, 2) + 1)


  s = lp.cost + lp.A' * dual_solution           # ＝ c + Aᵀy
  idx_lb = (lp.lb .> -Inf)
  idx_ub = (lp.ub .< +Inf)

  s_u = zeros(size(s))
  s_u[idx_ub] = max.(s[idx_ub], 0)      # sᵘ
  s_l = zeros(size(s))
  s_l[idx_lb] = max.(-s[idx_lb], 0)     # sˡ


  dual_residual = s - s_u + s_l                 # = 0 at optimum
  dual_feasibility = norm(dual_residual, 2) /
                     (norm(lp.cost, 2) + 1)

  primal_objective = Int(lp.sense) * lp.cost' * primal_solution + lp.offset
  dual_objective = Int(lp.sense) *
                   (lp.b' * dual_solution
                    +
                    lp.lb[idx_lb]' * s_l[idx_lb]     # ← 对应 sˡ
                    -
                    lp.ub[idx_ub]' * s_u[idx_ub])   # ← 对应 sᵘ
  +lp.offset

  duality_gap = abs(primal_objective - dual_objective) / (abs(primal_objective) + abs(dual_objective) + 1)

  kkt_error = sqrt(current_omega^2 * primal_feasibility^2 +
                   1 / current_omega^2 * dual_feasibility^2 +
                   duality_gap^2)
  ...
  return DataFrames.DataFrame(
    ...
    primal_feasibility=sqrt(sum(primal_feasibility .^ 2)),
...
function print_problem_details(lp::LinearProgrammingProblem)
  ...
  Printf.@printf(
    ...
    maximum(abs, nonzeros(lp.A)),
    minimum(abs, nonzeros(lp.A)),
    sum(abs, nonzeros(lp.A)) /
    length(nonzeros(lp.A))
  )

  col_norms = get_col_l_inf_norms(lp.A)
  row_norms = get_row_l_inf_norms(lp.A)
  ...
  Printf.@printf(
    ...
    maximum(abs, lp.cost),
    minimum(abs, lp.cost),
    sum(abs, lp.cost) / length(lp.cost)
  )
  ...
  Printf.@printf(
    ...
    maximum(abs, lp.b),
    minimum(abs, lp.b),
    sum(abs, lp.b) / length(lp.b)
  )

  bound_gaps = lp.ub - lp.lb
